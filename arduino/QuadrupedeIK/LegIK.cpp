...
LegIK::LegIK(int pinShoulder, int pinElbow) {
    this->shoulderPin = pinShoulder;
    this->elbowPin = pinElbow;
    Serial.println("Leg Online");
}
...
void LegIK::begin() {
    ...
    Serial.println("Leg Begin");
}

// Method implementation
void LegIK::printQuadrupedeIK() {
...
void LegIK::moveServos(int a1, int a2) {
...
// Raw servo angles
void LegIK::moveServosRaw(int a1, int a2) {
...
void LegIK::moveToCoordinates(float x, float y) {
    if (this->reverseX) {
        x *= -1; // Reverse the X axis for uniform control
    }
    if (this->reverseY) {
        y *= -1; // Reverse the X axis for uniform control
    }
...
float LegIK::normalize_angle_180(float angle) {
...
void LegIK::restrictMovements(float &x, float &y) {
...
void LegIK::getDistanceTarget(float x, float y, float limit, float &adjusted_length, float &_x, float &_y) {
...
void LegIK::getPointB(float ax, float ay, float cx, float cy, float sideC, int side, float &bx, float &by) {
...
void LegIK::IK(float x, float y, float &pointA_x, float &pointA_y, float &pointB_x, float &pointB_y, float &pointC_x, float &pointC_y) {
        pointA_x = 0;
        pointA_y = 0;
        ...
        getPointB(pointA_x, pointA_y, pointC_x, pointC_y, adjusted_length, side, pointB_x, pointB_y);
}

float LegIK::getShoulderAngle(float x, float y) {
...
void LegIK::trianglePointsToAngles(float pA_x, float pA_y, float pB_x, float pB_y, float pC_x, float pC_y, float &angle_AC, float &angle_AB, float &angle_BC) {
...
void LegIK::buildGaitFrame(int frames, int frame_number, float &longitudinalMovement, float &verticalMovement) {
...
int LegIK::cyclicalIndex(int idx, int limit) {
        idx = idx % limit;
        if (idx < 0) {
                idx += limit;
        }
...
void LegIK::runGait(int step) {
    this->gaitCycleIdx = step;
    float leg_x, leg_y;
    step = cyclicalIndex(step, this->gaitCycleLength);

    int leg_k_offset = (this->gaitCycleLength/this->gaitLegCount)*this->cycle_offset;
